...
**Tracker load hydration (`af214e4` → `d9c4ad6`):** D1 keeps live metrics only in Layer 4 (`api_cache`), never on the `companies` row. So on a fresh D1-mode reload the tracker would show all-dashes-until-Refresh-All. Fix: the `/full` response (fetched once per company on load) now carries `cachedStock` — the last-cached `api_cache` `stock_data`, **sanitized to market-only fields** (`CACHE_STOCK_STRIP` in the worker, since the cached blob is a whole-stock snapshot that can hold plaintext private fields). `_d1CompanyToTStock` merges it via the shared `_applyMarketData` helper, then applies `overriddenData` on top so manual overrides survive a reload. **Zero extra requests** (reuses the existing `/full` call). An earlier approach (`af214e4`) hydrated via a per-stock `cache-check` burst but that fired ~34 requests/load and tripped the `/api` rate limit — replaced. Live refresh stays on-demand (Refresh All / opening a company).

...
- [~] B3: Retire the sync key (phased; unblocked — Phase C recovery key + C3 both live). **B3a ✅** (`ce69164`, v39): `authDataHeaders()` token-primary — a provisioned device sends X-Auth-Token alone, sync key only as a no-token fallback; live-verified token-only auth works. **B3b-1 ✅** (`15f2577`, v40): removed the legacy connection-string login (parse/generate/share + #lock-login + loginFromDevice) — redundant with master-password login. 4 QA agents on B3a+B3b-1: all SHIP, 0 defects. **B3b-2 🔜** (legacy client-enc system + boot/lock rework — riskiest edit, own session, brick-hazard map in handoff memory). **B3c 🔜** (worker: drop X-Sync-Key auth + SYNC_SECRET + dead /sync routes — irreversible).

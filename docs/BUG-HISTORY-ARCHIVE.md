# Bug History — Archive (before 2026-07-24)

Body sections for QA categories that predate the current architecture. The cut-off is **Category 84
/ 2026-07-24**, when Security v2 closed and the app moved to token-only auth with envelope
encryption — everything from there on stays hot in `docs/BUG-HISTORY.md`.

**`docs/BUG-HISTORY.md` remains the index for BOTH files.** Its summary table lists every category
in the project, and `docs/check.sh` check 4 scans this file too, so a category documented here still
has to have a row there.

One oddity worth stating rather than leaving to be discovered: **Category 108 is here**, despite
having the highest number. Its content is from 2026-07-01 — it was written as an unindexed
`## Session 25` and only got a category number in 2026-08-19 (Cat 109). Archiving is by date, not by
number.
...
## Category 110 — Phase 16.3: Conviction Tracker (2026-07-01)

**Commit:** `c5f14b8` | **3 bugs found, 3 fixed**

### Bug 110.1 — Memory leak: conviction chart not destroyed on closeProfile
...
### Bug 110.2 — Missing null check in renderConvictionBadge
...
### Bug 110.3 — NaN conviction values from non-numeric input
...
## Category 111 — Phase 17.2: Position Sizing (2026-07-01)

**1 bug found, 1 fixed**

### Bug 111.1 — Position bar marker disappears when currentPct > barMax
...
## Category 112 — Phase 18.2: Earnings Calendar (2026-07-01)

**6 bugs found, 6 fixed**

### Bug 112.1 — CRITICAL: Wrong FMP API endpoint path
**Problem:** `v3/earning_calendar` under `/stable/` base URL produced invalid path. **Fix:** Changed to `earning-calendar`.

### Bug 112.2 — HIGH: Dashboard earnings status not interactive
...
### Bug 112.3 — LOW: Unused `_earningsCalendarCache` variable
**Fix:** Removed dead code.

### Bug 112.4 — MEDIUM: No rate-limit delay between sequential API calls
**Fix:** Added 300ms delay between FMP requests.

### Bug 112.5 — MEDIUM: `data.find` on unsorted array may select wrong upcoming date
**Fix:** Changed to `data.filter().sort()[0]` for correct nearest date.

### Bug 112.6 — MEDIUM: Missing try/catch around fetch loop
...
## Category 108 — Cross-Module Integration: Archive + Portfolio↔Pipeline Sync (2026-07-01)

> Renumbered 2026-08-19 (Cat 109) from `## Session 25`. It was the file's last heading outside the
> `## Category N` convention, indexed by nothing, and its number was already taken by a different
> topic (Category 25 — KNOWN-ISSUES Medium Sweep). Its 24 fixes were absent from the running total.
> Item numbers below changed 25.N → 108.N; content is unchanged.

### Archive System (Session 1)
| # | Bug | Fix | File:Line |
|---|-----|-----|-----------|
| 108.1 | D1 CHECK constraint rejects 'archived' pipeline value | Migrated live D1 table, added 'archived' to CHECK, normalized values to lowercase snake_case | index.html:7772 |
| 108.2 | archivedAt field lost on page refresh (not in preserve array) | Added 'archivedAt' to preserve array | index.html:8528 |
| 108.3 | setPipeline() doesn't set archivedAt when clicking Archived button | Added archivedAt management in setPipeline() | index.html:6341 |
| 108.4 | Screener Pipeline=Archived shows 0 results (pre-filtered) | Check screenerWantsArchived before filtering | index.html:8969 |

### Portfolio↔Pipeline Sync (Session 2)
| # | Bug | Fix | File:Line |
|---|-----|-----|-----------|
| 108.5 | deletePosition() doesn't trigger pipeline check | Added _checkSellAllPositions(delTicker) after position deletion | index.html:3886 |
| 108.6 | deleteTransaction() doesn't trigger pipeline check | Added _checkSellAllPositions(delTicker) after transaction deletion | index.html:4064 |
| 108.7 | CSV import ignores sold-all tickers (pipeline stays Owned) | Added post-import loop calling _checkSellAllPositions for zero-share tickers | index.html:4668 |
| 108.8 | saveTransaction() passes no name to _syncTickerToCompanies | Added tStocks[ticker]?.name||ticker as name parameter | index.html:4052 |
| 108.9 | Real estate/bond positions incorrectly synced to Companies tracker | Extended asset type guard to exclude real_estate and bond | index.html:3874 |
| 108.10 | Tracker UI not updated after sync (missing renderTracker calls) | Added renderTracker() to _syncTickerToCompanies, batch, and _checkSellAllPositions | index.html:8633 |
| 108.11 | String shares "0" treated as >0 in _checkSellAllPositions | Changed to parseFloat(p.shares)>0 | index.html:8669 |

### Review Reminders (Session 3)
| # | Bug | Fix | File:Line |
|---|-----|-----|-----------|
| 108.12 | saveReview() doesn't refresh dashboard widget or profile badge | Added renderDbReviewsDue() and renderReviewDueBadge() after save | index.html:11342 |
| 108.13 | Sort NaN when multiple companies have Infinity daysSince | Replaced subtraction with comparison-based sort | index.html:5421 |
| 108.14 | Light theme hover uses hardcoded dark color #252836 | Changed fallback to var(--border) | index.html:614 |
| 108.15 | Null review date produces NaN daysSince, silently hides badge | Added last.date truthiness check before Date computation | index.html:6726 |

### Summary Tab (Session 4)
| # | Bug | Fix | File:Line |
|---|-----|-----|-----------|
| 108.16 | + Note button clicks Preview instead of New Entry | Changed to call openResearchEntryForTicker() directly | index.html:6745 |
| 108.17 | Summary tab double-renders on every profile open | Removed redundant render in switchProfileTab | index.html:6214 |
| 108.18 | Dashboard reviews-due navigates to Overview unnecessarily | Removed switchProfileTab('overview') from onclick | index.html:5448 |
| 108.19 | P&L shows -100% when currentPrice is null | Fallback to avgCost when currentPrice missing | index.html:6761 |
| 108.20 | rv.id used unescaped in onclick handler | Added parseInt(rv.id)||0 sanitization | index.html:6834 |
| 108.21 | Hardcoded $ currency in positions/transactions | Use position/transaction currency field | index.html:6768 |

### Final QA Pass (cross-session)
| # | Bug | Fix | File:Line |
|---|-----|-----|-----------|
| 108.22 | doDeleteStock() crashes — researchNotes.filter() called on object not array | Iterate journal/news/market arrays, filter by .ticker not .companyTicker | index.html:8840 |
| 108.23 | D1 round-trip breaks pipeline — lowercase snake_case not normalized back to title case | Added case normalization map in _d1CompanyToTStock | index.html:8060 |
| 108.24 | deleteReview() doesn't refresh dashboard widget or profile badge | Added renderDbReviewsDue() and renderReviewDueBadge() after delete | index.html:11513 |
...
**Remaining (deferred to S2 cross-device completeness):** see `docs/KNOWN-ISSUES.md` § Sync Audit (SA.1–SA.5) and `docs/ROADMAP-ARCHIVE.md` § Data Sync Audit — localStorage-only fields → D1, non-stock positions cross-device, and soft-delete tombstones for framework/override/valuation.
